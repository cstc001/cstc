...
import org.cstceum.config.blockchain.OlympicConfig;
import org.cstceum.config.net.*;
import org.cstceum.core.Genesis;
import org.cstceum.core.genesis.GenesisConfig;
import org.cstceum.core.genesis.GenesisJson;
import org.cstceum.core.genesis.GenesisLoader;
import org.cstceum.crypto.ECKey;
import org.cstceum.net.p2p.P2pHandler;
import org.cstceum.net.rlpx.MessageCodec;
import org.cstceum.net.rlpx.Node;
import org.cstceum.util.BuildInfo;
...
/**
 * Utility class to retrieve property values from the cstceumj.conf files
 *
 ...
 * - resource cstceumj.conf : normally used as a reference config with default values
 *          and shouldn't be changed
 * - system property : each config entry might be altered via -D VM option
 * - [user dir]/config/cstceumj.conf
 * - config specified with the -Dcstceumj.conf.file=[file.conf] VM option
 * - CLI options
 *
...
public class SystemProperties {
    ...
    public SystemProperties(Config apiConfig, ClassLoader classLoader) {
        try {
            ...
            Config referenceConfig = ConfigFactory.parseResources("cstceumj.conf");
            logger.info("Config (" + (referenceConfig.entrySet().size() > 0 ? " yes " : " no  ") + "): default properties from resource 'cstceumj.conf'");
            String res = System.getProperty("cstceumj.conf.res");
            Config cmdLineConfigRes = res != null ? ConfigFactory.parseResources(res) : ConfigFactory.empty();
            logger.info("Config (" + (cmdLineConfigRes.entrySet().size() > 0 ? " yes " : " no  ") + "): user properties from -Dcstceumj.conf.res resource '" + res + "'");
            ...
            File userDirFile = new File(System.getProperty("user.dir"), "/config/cstceumj.conf");
            ...
            Config testConfig = ConfigFactory.parseResources("test-cstceumj.conf");
            logger.info("Config (" + (testConfig.entrySet().size() > 0 ? " yes " : " no  ") + "): test properties from resource 'test-cstceumj.conf'");
            ...
            String file = System.getProperty("cstceumj.conf.file");
            Config cmdLineConfigFile = file != null ? ConfigFactory.parseFile(new File(file)) : ConfigFactory.empty();
            logger.info("Config (" + (cmdLineConfigFile.entrySet().size() > 0 ? " yes " : " no  ") + "): user properties from -Dcstceumj.conf.file file '" + file + "'");
        ...
        if (blockchainConfig == null) {
            ...
            if (genesisJson.getConfig() != null && genesisJson.getConfig().isCustomConfig()) {
                ...
                if (config.hasPath("blockchain.config.name")) {
                    swicstc (config.getString("blockchain.config.name")) {
                        case "main":
                    ...
                    try {
                    ...
                    } cacstc (ClassNotFoundException e) {
                        ...
                        throw new RuntimeException("The class specified via blockchain.config.class '" + className + "' is not instance of org.cstceum.config.BlockchainForkConfig", e);
                    } cacstc (InstantiationException | IllegalAccessException e) {
