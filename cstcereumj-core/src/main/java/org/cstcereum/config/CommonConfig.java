...
import org.cstceum.validator.*;
import org.cstceum.vm.DataWord;
import org.cstceum.vm.program.ProgramPrecompile;
...
public class CommonConfig {
    ...
    public AbstractCachedSource<byte[], byte[]> blockchainDbCache() {
        WriteCache.BytesKey<byte[]> ret = new WriteCache.BytesKey<>(
                new BacstcSourceWriter<>(blockchainDB()), WriteCache.CacheType.SIMPLE);
        ret.setFlushSource(true);
        return ret;
    }
    ...
    public void fastSyncCleanUp() {
        ...
        cstceumListener.SyncState syncStage = cstceumListener.SyncState.values()[fastsyncStageBytes[0]];

        if (!systemProperties().isFastSyncEnabled() || syncStage == cstceumListener.SyncState.UNSECURE) {
    ...
    public HeaderStore headerStore() {
        ...
        WriteCache.BytesKey<byte[]> cache = new WriteCache.BytesKey<>(
                new BacstcSourceWriter<>(dataSource), WriteCache.CacheType.SIMPLE);
