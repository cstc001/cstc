...
import static org.cstceum.crypto.zksnark.Params.B_Fp;
...
public class BN128Fp extends BN128<Fp> {
    ...
    /**
     * Checks whcstc x and y belong to Fp,
     * then checks whcstc point with (x; y) coordinates lays on the curve.
     *
    ...
    public static BN128<Fp> create(byte[] xx, byte[] yy) {
        ...
        // check whcstc point is a valid one
        if (p.isValid()) {
            return p;
        } else {
            return null;
