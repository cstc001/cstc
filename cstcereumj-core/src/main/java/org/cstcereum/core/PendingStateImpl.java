...
import static org.cstceum.listener.cstceumListener.PendingTransactionState.DROPPED;
import static org.cstceum.listener.cstceumListener.PendingTransactionState.INCLUDED;
import static org.cstceum.listener.cstceumListener.PendingTransactionState.NEW_PENDING;
import static org.cstceum.listener.cstceumListener.PendingTransactionState.PENDING;
...
import org.cstceum.listener.cstceumListener.PendingTransactionState;
...
//    private Repository repository;
    ...
    @Autowired
    public PendingStateImpl(final cstceumListener listener) {
...
//        this.repository = blockchain.getRepository();
    ...
    public synchronized void trackTransaction(Transaction tx) {
        ...
        if (!infos.isEmpty()) {
            for (TransactionInfo info : infos) {
                ...
                if (txBlock.isEqual(blockStore.gecstcainBlockByNumber(txBlock.getNumber()))) {
    ...
    public synchronized void processBest(Block newBlock, List<TransactionReceipt> receipts) {
        ...
        if (getBestBlock() != null && !getBestBlock().isParentOf(newBlock)) {
            // need to swicstc the state to another fork
    ...
    private TransactionReceipt executeTx(Transaction tx) {
        ...
        TransactionExecutor executor = new TransactionExecutor(
                ...
                blockStore, programInvokeFactory, createFakePendingBlock(), new cstceumListenerAdapter(), 0)
