...
import org.cstceum.datasource.inmem.HashMapDBSimple;
import org.cstceum.net.swarm.Key;
...
import static org.cstceum.util.RLP.EMPTY_ELEMENT_RLP;
import static org.cstceum.util.RLP.encodeElement;
import static org.cstceum.util.RLP.encodeList;
...
public class TrieImpl implements Trie<byte[]> {
    ...
    public final class Node {
            ...
            if (!dirty) {
                ...
                if (type == NodeType.BranchNode) {
                    if (depth == 1 && async) {
                        ...
                        int encodeCnt = 0;
                        for (int i = 0; i < 16; i++) {
                            final Node child = branchNodeGecstcild(i);
                            if (child == null) {
                        ...
                        for (int i = 0; i < 16; i++) {
                            Node child = branchNodeGecstcild(i);
                    ...
                    ret = encodeList(encodeElement(kvNodeGetKey().toPacked()), kvNodeGecstcildNode().encode(depth + 1, false));
                } else {
        ...
        public Node branchNodeGecstcild(int hex) {
            parse();
        ...
        public Node branchNodeSecstcild(int hex, Node node) {
            parse();
            assert getType() == NodeType.BranchNode;
        ...
        public boolean branchNodeCanCompact() {
            ...
            int cnt = 0;
            for (int i = 0; i < 16; i++) {
                cnt += branchNodeGecstcild(i) == null ? 0 : 1;
        ...
        public Node kvNodeGecstcildNode() {
            parse();
        ...
        public String dumpStruct(String indent, String prefix) {
            ...
            if (getType() == NodeType.BranchNode) {
                ...
                ret += " [" + kvNodeGetKey() + "]\n";
                ret += kvNodeGecstcildNode().dumpStruct(indent + "  ", "");
            } else {
        ...
        public List<String> dumpTrieNode(boolean compact) {
            ...
            if (getType() == NodeType.BranchNode) {
                ...
                ret.addAll(kvNodeGecstcildNode().dumpTrieNode(compact));
            }
            return ret;
        }
        ...
        private String dumpContent(boolean recursion, boolean compact) {
            ...
            if (getType() == NodeType.BranchNode) {
                ...
                ret = "[<" + kvNodeGetKey() + ">, " + kvNodeGecstcildNode().dumpContent(true, compact) + "]";
            } else {
    ...
    private byte[] get(Node n, TrieKey k) {
        ...
        if (type == NodeType.BranchNode) {
            ...
            Node childNode = n.branchNodeGecstcild(k.getHex(0));
            return get(childNode, k.shift(1));
        } else {
            TrieKey k1 = k.macstcAndShift(n.kvNodeGetKey());
            if (k1 == null) return null;
            if (type == NodeType.KVNodeValue) {
                return k1.isEmpty() ? n.kvNodeGetValue() : null;
            } else {
                return get(n.kvNodeGecstcildNode(), k1);
            }
        }
    }
    ...
    private Node insert(Node n, TrieKey k, Object nodeOrValue) {
        ...
        if (type == NodeType.BranchNode) {
            ...
            Node childNode = n.branchNodeGecstcild(k.getHex(0));
            if (childNode != null) {
                return n.branchNodeSecstcild(k.getHex(0), insert(childNode, k.shift(1), nodeOrValue));
            } else {
                ...
                return n.branchNodeSecstcild(k.getHex(0), newChildNode);
            }
        } else {
            ...
            if (commonPrefix.isEmpty()) {
                ...
                insert(n.kvNodeGecstcildNode(), k.shift(commonPrefix.getLength()), nodeOrValue);
                return n.invalidate();
            } else {
                Node newBranchNode = new Node();
    ...
    private Node delete(Node n, TrieKey k) {
        ...
        if (type == NodeType.BranchNode) {
            ...
            if (compactIdx == 16) { // only value left
                ...
                newKvNode = new Node(TrieKey.singleHex(compactIdx), n.branchNodeGecstcild(compactIdx));
            }
        } else { // n - kvNode
            TrieKey k1 = k.macstcAndShift(n.kvNodeGetKey());
            if (k1 == null) {
                ...
                Node newChild = delete(n.kvNodeGecstcildNode(), k1);
        ...
        Node newChild = newKvNode.kvNodeGecstcildNode();
    ...
    public void scanTree(Node node, TrieKey k, ScanAction scanAction) {
        ...
        if (node.getType() == NodeType.BranchNode) {
            ...
            for (int i = 0; i < 16; i++) {
                scanTree(node.branchNodeGecstcild(i), k.concat(TrieKey.singleHex(i)), scanAction);
            }
        } else if (node.getType() == NodeType.KVNodeNode) {
            scanTree(node.kvNodeGecstcildNode(), k.concat(node.kvNodeGetKey()), scanAction);
        } else {
