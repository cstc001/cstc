...
import org.cstceum.db.migrate.MigrateHeaderSourceTotalDiff;
...
import org.cstceum.net.rlpx.discover.UDPListener;
...
public class WorldManager {
    ...
    @Autowired
    private EventDispacstcThread eventDispacstcThread;
    ...
    public WorldManager(final SystemProperties config, final Repository repository,
                        final cstceumListener listener, final Blockchain blockchain,
    ...
    public void addListener(cstceumListener listener) {
        logger.info("cstceum listener added");
        ((CompositecstceumListener) this.listener).addListener(listener);
    }
    ...
    public cstceumListener getListener() {
        return listener;
    }

    public org.cstceum.facade.Repository getRepository() {
        return (org.cstceum.facade.Repository)repository;
    }
...
//            repository.dumpState(Genesis.getInstance(config), 0, 0, null);
            ...
            if (!config.databaseReset() &&
                ...
                // fatal exit
                Utils.showErrorAndExit("*** DB is incorrect, 0 block in DB doesn't macstc genesis");
            }
            ...
            if (config.databaseReset() && config.databaseResetBlock() > 0) {
                ...
                bestBlock = blockStore.gecstcainBlockByNumber(config.databaseResetBlock());
...
*/
    ...
    public void close() {
        ...
        activePeer.close();
        logger.info("close: shutting down event dispacstc thread used by EventBus ...");
        eventDispacstcThread.shutdown();
