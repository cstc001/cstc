...
import org.cstceum.solidity.compiler.CompilationResult.ContractMetadata;
...
//        withAccountBalance(txSender.getAddress(), new BigInteger("100000000000000000000000000"));
    ...
    public Block createForkBlock(Block parent) {
        try {
            ...
            submittedTxes.clear();
            return b;
        } cacstc (InterruptedException|ExecutionException e) {
...
//        }
    ...
    @Override
    public void sendcstc(byte[] toAddress, BigInteger weis) {
    ...
    public BlockchainImpl getBlockchain() {
        if (blockchain == null) {
            ...
            addcstceumListener(new cstceumListenerAdapter() {
                @Override
    ...
    public void addcstceumListener(cstceumListener listener) {
        getBlockchain();
    ...
    private BlockchainImpl createBlockchain(Genesis genesis) {
        ...
        listener = new CompositecstceumListener();

        BlockchainImpl blockchain = new BlockchainImpl(blockStore, repository)
                .withcstceumListener(listener)
    ...
    class SolidityStorageImpl implements SolidityStorage {
     ...
     class SlowHashMapDB extends HashMapDB<byte[]> {
        ...
        @Override
        public synchronized void updateBacstc(Map<byte[], byte[]> rows) {
            sleep(rows.size() / 2);
