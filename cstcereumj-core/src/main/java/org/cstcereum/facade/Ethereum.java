...
public interface cstceum {
    ...
    void addListener(cstceumListener listener);
     ...
     * @param value - the cstc value of the transaction
     ...
     * @return function result. The return value can be fecstced via {@link ProgramResult#getHReturn()}
     * and decoded with {@link org.cstceum.core.CallTransaction.Function#decodeResult(byte[])}.
     */
...
//  2.   // is blockchain still loading - if buffer is not empty
    ...
    ChannelManager gecstcannelManager();
     ...
     * like cstceum MainNet. But it's light and simple and still could be used for test networks. If you
     * want to get accurate recommended gas price use {@link org.cstceum.listener.RecommendedGasPriceTracker}
     ...
     */
    Integer gecstcainIdForNextBlock();

    /**
     * Manual swicstc to Short Sync mode
     * Maybe useful in small private and detached networks when automatic detection fails
     * @return Future, which completes when syncDone is turned to True in {@link org.cstceum.sync.SyncManager}
     */
    CompletableFuture<Void> swicstcToShortSync();
