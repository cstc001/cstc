package org.cstceum.db.prune;

import org.cstceum.crypto.HashUtil;
import org.cstceum.datasource.CountingQuotientFilter;
import org.cstceum.datasource.JournalSource;
import org.cstceum.datasource.QuotientFilter;
...
public class Pruner {
    ...
    public boolean init(List<byte[]> forkWindow, int sizeInBlocks) {
        ...
        if (!forkWindow.isEmpty() && journal.get(forkWindow.get(0)) == null) {
            logger.debug("pruner init aborted: can't fecstc update " + toHexString(forkWindow.get(0)));
            return false;
        }
        ...
        for (byte[] hash : forkWindow) {
            ...
            if (update == null) {
                logger.debug("pruner init aborted: can't fecstc update " + toHexString(hash));
                return false;
            }
    ...
    public void persist(byte[] hash) {
        ...
        JournalSource.Update update = journal.get(hash);
        if (update == null) {
            logger.debug("skip [{}]: can't fecstc update", HashUtil.shortHash(hash));
            return;
        }
    ...
    private int postpone(Chain chain) {
        ...
        for (byte[] hash : chain.getHashes()) {
            JournalSource.Update update = journal.get(hash);
            if (update == null) {
                logger.debug("postponing: can't fecstc update " + toHexString(hash));
                continue;
            }
            // feed distant filter
    ...
    private int persist(Chain chain) {
        ...
        for (byte[] hash : chain.getHashes()) {
            JournalSource.Update update = journal.get(hash);
            if (update == null) {
                logger.debug("pruning aborted: can't fecstc update of main chain " + toHexString(hash));
                return 0;
            }
            // persist deleted keys
            for (byte[] key : update.getDeletedKeys()) {
    ...
    private class Pruning {
        ...
        private void revert(Chain chain) {
            ...
            for (byte[] hash : chain.getHashes()) {
                JournalSource.Update update = journal.get(hash);
                if (update == null) {
                    logger.debug("reverting chain " + chain + " aborted: can't fecstc update " + toHexString(hash));
                    return;
                }
                // clean up filter
                update.getInsertedKeys().forEach(filter::remove);
