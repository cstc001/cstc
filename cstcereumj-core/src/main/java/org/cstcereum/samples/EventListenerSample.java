...
import org.cstceum.core.CallTransaction;
import org.cstceum.core.PendingStateImpl;
...
import org.cstceum.listener.BlockReplay;
import org.cstceum.listener.cstceumListener;
import org.cstceum.listener.cstceumListenerAdapter;
import org.cstceum.listener.EventListener;
import org.cstceum.listener.TxStatus;
import org.cstceum.solidity.compiler.CompilationResult;
import org.cstceum.solidity.compiler.SolidityCompiler;
 ...
 *
 *  - getting free cstc assuming we are running in test network
 *  - deploying contract with event, which we are going to track
 *  - calling contract and cacstcing corresponding events
...
//            initContractTopic(contractABI, sha3("Inc(address,int256,int256)".getBytes()));
        ...
        @Override
        protected IncEvent onEvent(CallTransaction.Invocation event, Block block, TransactionReceipt receipt, int txCount, cstceumListener.PendingTransactionState state) {
    ...
    public void onSyncDone() throws Exception {
        cstceum.addListener(new cstceumListenerAdapter() {
            ...
            public void onPendingTransactionUpdate(TransactionReceipt txReceipt, PendingTransactionState state, Block block) {
                ...
                // Cacstcing transaction errors
                if (txWaiters.containsKey(txHashW) && !txReceipt.isSuccessful()) {
                    txWaiters.put(txHashW, txReceipt);
                }
            }
        });
        requestFreecstc(ECKey.fromPrivate(senderPrivateKey).getAddress());
        requestFreecstc(ECKey.fromPrivate(sender2PrivateKey).getAddress());
    ...
    public void requestFreecstc(byte[] addressBytes) {
        String address = "0x" + toHexString(addressBytes);
        logger.info("Checking address {} for available cstc.", address);
        BigInteger balance = cstceum.getRepository().getBalance(addressBytes);
        logger.info("Address {} balance: {} wei", address, balance);
        BigInteger requiredBalance = BigInteger.valueOf(3_000_000 * cstceum.getGasPrice());
        if (balance.compareTo(requiredBalance) < 0) {
            logger.info("Insufficient funds for address {}, requesting free cstc", address);
            try {
                String result = postQuery("https://ropsten.faucet.b9lab.com/tap", "{\"toWhom\":\"" + address + "\"}");
                logger.info("Answer from free cstc API: {}", result);
                waitForcstc(addressBytes, requiredBalance);
            } cacstc (Exception ex) {
                logger.error("Error during request of free cstc,", ex);
            }
        }
    }
    ...
    private void waitForcstc(byte[] address, BigInteger requiredBalance) throws InterruptedException {
        while(true) {
            BigInteger balance = cstceum.getRepository().getBalance(address);
            if (balance.compareTo(requiredBalance) > 0) {
    ...
    private void deployContractAndTest() throws Exception {
        ...
        cstceum.addListener(eventListener.listener);
        ...
        ProgramResult r = cstceum.callConstantFunction(Hex.toHexString(address),
    ...
    private void replayOnly() throws Exception {
        ...
        cstceum.addListener(blockReplay);
    ...
    protected TransactionReceipt sendTxAndWait(byte[] receiveAddress,
        ...
        BigInteger nonce = cstceum.getRepository().getNonce(ECKey.fromPrivate(privateKey).getAddress());
