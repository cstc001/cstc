...
import org.cstceum.util.ByteArrayMap;
...
public class SyncQueueImpl implements SyncQueueIfc {
    ...
    class HeaderElement {
        ...
        public List<HeaderElement> gecstcildren() {
    ...
    List<HeaderElement> getLongescstcain() {
    ...
    private boolean hasGaps() {
        List<HeaderElement> longescstcain = getLongescstcain();
        return longescstcain.get(longescstcain.size() - 1).header.getNumber() < maxNum;
    }

    private void trimChain() {
        List<HeaderElement> longescstcain = getLongescstcain();
        if (longescstcain.size() > MAX_CHAIN_LEN) {
            long newTrimNum = getLongescstcain().get(longescstcain.size() - MAX_CHAIN_LEN).header.getNumber();
            for (int i = 0; darkZoneNum < newTrimNum; darkZoneNum++, i++) {
                ByteArrayWrapper wHash = new ByteArrayWrapper(longescstcain.get(i).header.getHash());
                putGenHeaders(darkZoneNum, Collections.singletonMap(wHash, longescstcain.get(i)));
            }
            darkZoneNum--;
        }
    }
    ...
    private List<HeadersRequest> requestHeadersImpl(int count, int maxRequests, int maxTotHeaderCount) {
        ...
        if (hasGaps()) {
            List<HeaderElement> longescstcain = getLongescstcain();
            startNumber = longescstcain.get(longescstcain.size() - 1).header.getNumber();
...
//            this.handler = handler;
        ...
        public T traverse(HeaderElement el) {
            ...
            for (HeaderElement child : el.gecstcildren()) {
