...
import org.cstceum.datasource.rocksdb.RocksDbDataSource;
import org.cstceum.db.DbFlushManager;
import org.cstceum.db.HeaderStore;
import org.cstceum.db.IndexedBlockStore;
...
import org.cstceum.net.client.Capability;
import org.cstceum.net.eth.handler.Eth63;
import org.cstceum.net.message.ReasonCode;
import org.cstceum.net.server.Channel;
import org.cstceum.trie.TrieKey;
import org.cstceum.util.*;
...
import static org.cstceum.listener.cstceumListener.SyncState.COMPLETE;
import static org.cstceum.listener.cstceumListener.SyncState.SECURE;
import static org.cstceum.listener.cstceumListener.SyncState.UNSECURE;
import static org.cstceum.trie.TrieKey.fromPacked;
import static org.cstceum.util.CompactEncoder.hasTerminator;
...
public class FastSyncManager {
    ...
    @Autowired
    CompositecstceumListener listener;
    ...
    public SyncStatus getSyncState() {
        ...
        cstceumListener.SyncState syncStage = getSyncStage();
        swicstc (syncStage) {
            case UNSECURE:
...
//                idle.getEthHandler().setStatus(SyncState.IDLE);
    ...
    private void setSyncStage(cstceumListener.SyncState stage) {
        if (stage == null) {
    ...
    private cstceumListener.SyncState getSyncStage() {
        byte[] bytes = blockchainDB.get(FASTSYNC_DB_KEY_SYNC_STAGE);
        if (bytes == null) return UNSECURE;
        return cstceumListener.SyncState.values()[bytes[0]];
    }
    ...
    private void syncUnsecure(BlockHeader pivot) {
        ...
        final CountDownLacstc syncDoneLacstc = new CountDownLacstc(1);
        listener.addListener(new cstceumListenerAdapter() {
            @Override
            public void onSyncDone(SyncState state) {
                syncDoneLacstc.countDown();
            }
        });
...
//        }
    ...
    private void syncSecure() {
        ...
        setSyncStage(cstceumListener.SyncState.SECURE);
        ...
        if (!FastByteComparisons.equal(headersDownloader.getGenesisHash(), config.getGenesis().getHash())) {
            logger.error("FASTSYNC FATAL ERROR: after downloading header chain starting from the pivot block (" +
                    pivot.getShortDescr() + ") obtained genesis block doesn't macstc ours: " + toHexString(headersDownloader.getGenesisHash()));
            logger.error("Can't recover and exiting now. You need to restart from scracstc (all DBs will be reset)");
            System.exit(-666);
        }
        dbFlushManager.commit();
        dbFlushManager.flush();
    ...
    private void syncBlocksReceipts() {
        ...
        if (!config.fastSyncSkipHistory()) {
            ...
            setSyncStage(cstceumListener.SyncState.COMPLETE);
    ...
    public boolean removeHeadersDb(Logger logger) {
        if (blockStore.getBestBlock().getNumber() > 0 &&
                blockStore.gecstcainBlockByNumber(1) != null) {
    ...
    public void main() {
        ...
        if (blockchain.getBestBlock().getNumber() == 0 || getSyncStage() == SECURE || getSyncStage() == COMPLETE) {
            ...
            try {
                cstceumListener.SyncState origSyncStage = getSyncStage();

                swicstc (origSyncStage) {
                    case UNSECURE:
                        pivot = getPivotBlock();
                        if (pivot.getNumber() == 0) {
                            logger.info("FastSync: too short blockchain, proceeding with regular sync...");
                            syncManager.initRegularSync(cstceumListener.SyncState.COMPLETE);
                            return;
                        }
                    ...
                    case SECURE:
                        if (origSyncStage == SECURE) {
                            ...
                            syncManager.initRegularSync(cstceumListener.SyncState.UNSECURE);
                        }

                        syncSecure();

                        listener.onSyncDone(cstceumListener.SyncState.SECURE);
                    case COMPLETE:
                        if (origSyncStage == COMPLETE) {
                            logger.info("FastSync: SECURE sync was completed prior to this run, proceeding with next stage...");
                            logger.info("Initializing regular sync");
                            syncManager.initRegularSync(cstceumListener.SyncState.SECURE);
                        }

                        syncBlocksReceipts();

                        listener.onSyncDone(cstceumListener.SyncState.COMPLETE);
                }
                logger.info("FastSync: Full sync done.");
            } cacstc (InterruptedException ex) {
    ...
    private BlockHeader getPivotBlock() throws InterruptedException {
        ...
        if (pivotBlockHash != null) {
            logger.info("FastSync: fecstcing trusted pivot block with hash " + toHexString(pivotBlockHash));
        } else {
            ...
            logger.info("FastSync: fecstcing pivot block #" + pivotBlockNumber);
        }
        ...
        try {
            while (true) {
                ...
                long t = System.currentTimeMillis();
                if (t - s > 5000) {
                    logger.info("FastSync: waiting for a peer to fecstc pivot block...");
                    s = t;
                }
            ...
            try {
                ...
                if (!blockHeaders.isEmpty()) {
                    ...
                    if (FastByteComparisons.equal(pivotBlockHash, ret.getHash())) {
                        logger.info("Pivot header fecstced: " + ret.getShortDescr());
                        return ret;
                    }
     ...
     *             BlockHeader, null - if pivot successfully fecstced and verified by majority of peers
     */
            ...
            try {
                ...
                for (Map.Entry<BlockHeader, Integer> pivotEntry : pivotMap.entrySet()) {
                    ...
                    if (pivotEntry.getValue() * 2 > peerCount) {
                        logger.info("Pivot header fecstced: " + pivotEntry.getKey().getShortDescr());
