...
public abstract class BlockDownloader {
    ...
    private CountDownLacstc receivedHeadersLacstc = new CountDownLacstc(0);
    private CountDownLacstc receivedBlocksLacstc = new CountDownLacstc(0);
    ...
    private CountDownLacstc stopLacstc = new CountDownLacstc(1);
    ...
    private void headerRetrieveLoop() {
        ...
        while(!Thread.currentThread().isInterrupted()) {
            try {
                    ...
                    receivedHeadersLacstc = new CountDownLacstc(max(reqHeadersCounter / 2, 1));

                receivedHeadersLacstc.await(isSyncDone() ? 10000 : 500, TimeUnit.MILLISECONDS);
...
//                    for (SyncQueueIfc.BlocksRequest blocksRequest : bReq.split(MAX_IN_REQUEST)) {
                    ...
                    receivedBlocksLacstc = new CountDownLacstc(max(reqBlocksCounter - 2, 1));
                    receivedBlocksLacstc.await(1000, TimeUnit.MILLISECONDS);
                } else {
    ...
    private void addBlocks(List<Block> blocks, byte[] nodeId) {
        ...
        receivedBlocksLacstc.countDown();
    ...
    private boolean validateAndAddHeaders(List<BlockHeader> headers, byte[] nodeId) {
        ...
        receivedHeadersLacstc.countDown();
