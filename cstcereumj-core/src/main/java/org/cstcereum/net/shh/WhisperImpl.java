...
public class WhisperImpl extends Whisper {
    ...
    private Set<MessageWacstcer> filters = new HashSet<>();
    ...
    private void macstcMessage(WhisperMessage m) {
        for (MessageWacstcer f : filters) {
            if (f.macstc(m.getTo(), m.getFrom(), m.getTopics())) {
                f.newMessage(m);
            }
        }
    }
