 ...
 */
package org.cstceum.net.message;
...
import org.cstceum.net.p2p.*;
import org.spongycastle.util.encoders.Hex;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Component;

import java.util.List;
import java.util.regex.Macstcer;
...
public class StaticMessages {
    ...
    private String buildHelloAnnouncement() {
        ...
        Macstcer macstcer = pattern.macstcer(numberVersion);
        if (macstcer.find()) {
            numberVersion = numberVersion.substring(macstcer.start(), macstcer.end());
        }
        ...
        return String.format("cstceum(J)/v%s/%s/%s/Java/%s", numberVersion, system,
