...
import static org.cstceum.datasource.MemSizeEstimator.ByteArrayEstimator;
import static org.cstceum.net.eth.EthVersion.V62;
import static org.cstceum.net.message.ReasonCode.USELESS_PEER;
import static org.cstceum.sync.PeerState.*;
import static org.cstceum.sync.PeerState.BLOCK_RETRIEVING;
import static org.cstceum.util.Utils.longToTimePeriod;
...
public class Eth62 extends EthHandler {
    ...
    public Eth62(final SystemProperties config, final Blockchain blockchain,
                 final BlockStore blockStore, final CompositecstceumListener cstceumListener) {
        this(version, config, blockchain, blockStore, cstceumListener);
    }

    Eth62(final EthVersion version, final SystemProperties config,
          final Blockchain blockchain, final BlockStore blockStore,
          final CompositecstceumListener cstceumListener) {
        super(version, config, blockchain, blockStore, cstceumListener);
    }
    ...
    protected synchronized void processStatus(StatusMessage msg, ChannelHandlerContext ctx) throws InterruptedException {
        ...
        try {
            ...
            cstceumListener.onEthStatusUpdated(channel, msg);
        ...
        } cacstc (NoSuchElementException e) {
    ...
    protected synchronized void processTransactions(TransactionsMessage msg) {
        ...
        if (!newPending.isEmpty()) {
            TransactionTask transactionTask = new TransactionTask(newPending, channel.gecstcannelManager(), channel);
    ...
    @Override
    public synchronized void fecstcBodies(List<BlockHeaderWrapper> headers) {
        syncStats.reset();
