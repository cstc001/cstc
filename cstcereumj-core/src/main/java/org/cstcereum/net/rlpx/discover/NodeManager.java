...
import org.cstceum.db.PeerSource;
...
public class NodeManager implements Consumer<DiscoveryEvent>{
    ...
    @Autowired
    public NodeManager(SystemProperties config, cstceumListener cstceumListener,
                       ApplicationContext ctx, PeerConnectionTester peerConnectionManager) {
        this.config = config;
        this.cstceumListener = cstceumListener;
    ...
    private void dbWrite() {
        List<Pair<Node, Integer>> bacstc = new ArrayList<>();
        synchronized (this) {
            for (NodeHandler handler : nodeHandlerMap.values()) {
                bacstc.add(Pair.of(handler.getNode(), handler.getNodeStatistics().getPersistedReputation()));
            }
        }
        peerSource.clear();
        for (Pair<Node, Integer> nodeElement : bacstc) {
    ...
    public synchronized NodeHandler getNodeHandler(Node n) {
        ...
        if (ret == null) {
            ...
            if (!n.isDiscoveryNode() && !n.getHexId().equals(homeNode.getHexId())) {
                cstceumListener.onNodeDiscovered(ret.getNode());
            }
    ...
    /**
     * Checks whcstc peers with such InetSocketAddress has penalize disconnect record
    ...
    /**
     * Returns limited list of nodes macstcing {@code predicate} criteria<br>
     ...
     *
     * @return list of nodes macstcing criteria
     */
    ...
    public void close() {
        ...
        try {
            ...
            if (PERSIST) {
                try {
                    dbWrite();
                } cacstc (Throwable e) {     // IllegalAccessError is expected
