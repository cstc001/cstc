...
import org.cstceum.util.blockchain.SolidityContract;
...
import static org.cstceum.util.ByteUtil.intToBytes;
import static org.cstceum.util.blockchain.cstcUtil.Unit.cstc;
import static org.cstceum.util.blockchain.cstcUtil.convert;
...
//        System.out.println("Gen root: " + Hex.toHexString(bc.getBlockchain().getBestBlock().getStateRoot()));
        ...
        bc.sendcstc(alice.getAddress(), convert(3, cstc));
        ...
        bc.sendcstc(bob.getAddress(), convert(5, cstc));
        bc.createBlock();
        ...
        Assert.assertEquals(convert(30, cstc), bc.getBlockchain().getRepository().getBalance(alice.getAddress()));
        Assert.assertEquals(convert(50, cstc), bc.getBlockchain().getRepository().getBalance(bob.getAddress()));

        {
            ...
            Assert.assertEquals(convert(3 * 9, cstc), r1.getBalance(alice.getAddress()));
            Assert.assertEquals(convert(5 * 9, cstc), r1.getBalance(bob.getAddress()));
        }

        {
            Block b1 = bc.getBlockchain().getBlockByNumber(bestBlockNum - 2);
            Repository r1 = bc.getBlockchain().getRepository().getSnapshotTo(b1.getStateRoot());
            Assert.assertEquals(convert(3 * 8, cstc), r1.getBalance(alice.getAddress()));
            Assert.assertEquals(convert(5 * 8, cstc), r1.getBalance(bob.getAddress()));
        }

        {
            Block b1 = bc.getBlockchain().getBlockByNumber(bestBlockNum - 3);
            Repository r1 = bc.getBlockchain().getRepository().getSnapshotTo(b1.getStateRoot());
            Assert.assertEquals(convert(3 * 7, cstc), r1.getBalance(alice.getAddress()));
            Assert.assertEquals(convert(5 * 7, cstc), r1.getBalance(bob.getAddress()));
        }
...
//        repository.getDetailsDataStore().setStorageDS(storageDS);
    ...
    public void rewriteSameTrieNode() throws Exception {
        ...
        bc.sendcstc(receiver, BigInteger.valueOf(0x77777777));
        bc.createBlock();

        for (int i = 0; i < 100; i++) {
            bc.sendcstc(new ECKey().getAddress(), BigInteger.valueOf(i));
        }
