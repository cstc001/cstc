...
import static org.cstceum.listener.cstceumListener.PendingTransactionState.*;
...
public class PendingStateTest {
    ...
    static class PendingListener extends cstceumListenerAdapter {
...
//        public BlockingQueue<Triple<TransactionReceipt, PendingTransactionState, Block>> onPendingTransactionUpdate = new LinkedBlockingQueue<>();
    ...
    public void testSimple() throws InterruptedException {
        ...
        bc.addcstceumListener(l);
        Triple<TransactionReceipt, cstceumListener.PendingTransactionState, Block> txUpd;
        PendingStateImpl pendingState = (PendingStateImpl) bc.getBlockchain().getPendingState();

        ECKey alice = new ECKey();

        bc.sendcstc(new byte[20], BigInteger.valueOf(100000));
        bc.sendcstc(new byte[20], BigInteger.valueOf(100000));
    ...
    public void testRebranch1() throws InterruptedException {
        ...
        PendingListener l = new PendingListener();
        bc.addcstceumListener(l);
        Triple<TransactionReceipt, cstceumListener.PendingTransactionState, Block> txUpd = null;
        PendingStateImpl pendingState = (PendingStateImpl) bc.getBlockchain().getPendingState();
        ...
        bc.sendcstc(bob.getAddress(), convert(100, cstc));
        bc.sendcstc(charlie.getAddress(), convert(100, cstc));
    ...
    public void testRebranch2() throws InterruptedException {
        ...
        bc.submitTransaction(tx1);
        bc.sendcstc(alice.getAddress(), BigInteger.valueOf(1000000));
        Block b2 = bc.createBlock();
    ...
    public void testRejected1() throws InterruptedException {
        ...
        for (int i = 0; i < 16; i++) {
            bc.createBlock();
            cstceumListener.PendingTransactionState state = l.pollTxUpdateState(tx1);
            if (state == cstceumListener.PendingTransactionState.DROPPED) {
                break;
            }
            if (i == 15) {
    ...
    public void testInvalidTransaction() throws InterruptedException {
        StandaloneBlockchain bc = new StandaloneBlockchain();
        final CountDownLacstc txHandle = new CountDownLacstc(1);
        PendingListener l = new PendingListener() {
            @Override
            public void onPendingTransactionUpdate(TransactionReceipt txReceipt, PendingTransactionState state, Block block) {
