...
import org.cstceum.core.BlockIdentifier;
...
import org.cstceum.mine.MinerListener;
import org.cstceum.net.eth.message.EthMessage;
import org.cstceum.net.eth.message.EthMessageCodes;
import org.cstceum.net.eth.message.NewBlockHashesMessage;
import org.cstceum.net.eth.message.NewBlockMessage;
import org.cstceum.net.eth.message.StatusMessage;
import org.cstceum.net.eth.message.TransactionsMessage;
...
public class BlockTxForwardTest {
    ...
    static class MinerNode extends BasicSample implements MinerListener{
        ...
        public void run() {
            if (config.isMineFullDataset()) {
                ...
                Ethash ethash = Ethash.getForBlock(config, cstceum.getBlockchain().getBestBlock().getNumber());
                ethash.getFullDataset();
                logger.info("Full dataset generated (loaded).");
            }
            cstceum.getBlockMiner().addListener(this);
            cstceum.getBlockMiner().startMining();
        }
    ...
    static class GeneratorNode extends BasicSample {
        ...
        private void generateTransactions() throws Exception{
            ...
            for (int i = cstceum.getRepository().getNonce(senderKey.getAddress()).intValue(), j = 0; j < 20000; i++, j++) {
                {
    ...
    /**
     *  Creating 3 cstceumJ instances with different config classes
    ...
    public void testTest() throws Exception {
        ...
        testLogger.info("Starting cstceumJ miner instance!");
        cstceum miner = cstceumFactory.createcstceum(MinerConfig.class);

        miner.addListener(new cstceumListenerAdapter() {
            @Override
            ...
            public void onRecvMessage(Channel channel, Message message) {
                ...
                swicstc (((EthMessage) message).getCommand()) {
        ...
        cstceumFactory.createcstceum(RegularConfig.class);

        testLogger.info("Starting cstceumJ txSender instance!");
        cstceum txGenerator = cstceumFactory.createcstceum(GeneratorConfig.class);
        txGenerator.addListener(new cstceumListenerAdapter() {
            @Override
            public void onRecvMessage(Channel channel, Message message) {
                super.onRecvMessage(channel, message);
                if (!(message instanceof EthMessage)) return;
