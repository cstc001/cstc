...
import org.cstceum.net.swarm.bzz.BzzMessage;
import org.cstceum.net.swarm.bzz.BzzProtocol;
import org.cstceum.net.swarm.bzz.PeerAddress;
...
//        Thread.sleep(3000);
    ...
    private String dumpPeers(TestPeer[] allPeers, Key key) {
        String s = "Name\cstcunks\tPeers\tMsgIn\tMsgOut\n";
...
//            stdout.setFilter(Hex.toHexString(key.getBytes()));
        ...
        System.out.println("Name\cstcunks\tPeers\tMsgIn\tMsgOut");
        for (TestPeer peer : allPeers) {
...
//        TestPeer.MessageOut = true;
        ...
        int chunks = 100;
        byte[] data = new byte[(int) (chunks * chunker.gecstcunkSize())];
        for (int i = 0; i < chunks; i++) {
            for (int idx = (int) (i * chunker.gecstcunkSize()); idx < (i+1) * chunker.gecstcunkSize(); idx++) {
                data[idx] = (byte) (i + 1);
            }
        }
