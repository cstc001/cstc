...
import org.cstceum.net.shh.MessageWacstcer;
import org.cstceum.net.shh.WhisperMessage;
...
//        Thread.sleep(1000000000);
        ...
        final CountDownLacstc semaphoreBlocks = new CountDownLacstc(1);
        final CountDownLacstc semaphoreFirstBlock = new CountDownLacstc(1);
        ...
        String identity1 = cstceum1.getWhisper().newIdentity();
        String identity2 = cstceum2.getWhisper().newIdentity();

        final int[] counter1 = new int[1];
        final int[] counter2 = new int[1];

        cstceum1.getWhisper().wacstc(new MessageWacstcer(identity1, null, null) {
            @Override
        ...
        cstceum2.getWhisper().wacstc(new MessageWacstcer(identity2, null, null) {
            @Override
            protected void newMessage(WhisperMessage msg) {
        ...
        while (semaphoreBlocks.getCount() > 0) {
            cstceum1.getWhisper().send(identity2, "Hello Eth2!".getBytes(), null);
            cstceum2.getWhisper().send(identity1, "Hello Eth1!".getBytes(), null);
            cnt++;
